...
function M.get_cursor_pos()
  ...
  if not pos then error("Failed to get cursor position") end
...
function M.get_selection_anchor_pos()
  ...
  if not pos then error("Failed to get selection anchor position") end
  return { row = pos[2], col = pos[3], buf = pos[1] }
end
...
function M.get_last_selection_pos()
  local start_pos = vim.fn.getpos("'<")
  local end_pos = vim.fn.getpos("'>")
  if not start_pos or not end_pos then
    error("Failed to get last selection positions")
  end
...
--
---@return EditorMode
function M.get_mode() return vim.api.nvim_get_mode() end
...
function M.get_visual_selection(opts)
  ...
  if M.get_mode().blocking then error("Not implemented for block mode") end
  ...
  elseif mode == "v" then
    ...
    if
      ...
      or (
        cursor_pos.col == selection_anchor.row
        and cursor_pos.col > selection_anchor.col
      )
    then
      start_pos = selection_anchor
      end_pos = cursor_pos
    ...
    start_pos, end_pos = M.get_last_selection_pos()
  end

  local lines = vim.fn.getline(start_pos.row, end_pos.row)
  if not lines then error("Failed to get current selection") end
  ---@cast lines string[]

  if #lines == 0 then return {} end

  if mode == "V" then return lines end
